...
**Dependency wiring**: 
- tests_root.zig uses `_ = @import(...)` pattern (file collection, not unit tests)
...
- Semantic "fallback" uses in terminal_profile.zig (data value, not code pattern) are intentional

## Status

Test infrastructure present but no executable tests yet. File collection serves as build-time compilation check only. Component testing deferred to integration harness phase.
